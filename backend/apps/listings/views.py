...
from .models import Car, CarImage, CarStatus, Request, RequestStatus, RequestStatusEvent
...
class MyCarStatusView(APIView):
    ...
    def post(self, request, car_id):
        ...
        if new_status not in allowed:
            return Response(
                {"detail": f"Cannot transition from '{car.status}' to '{new_status}'."},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        car.save(update_fields=["status", "admin_note", "published_at", "updated_at"])
...
class AdminCarStatusView(APIView):
    ...
    def post(self, request, car_id):
        ...
        elif new_status == "needs_changes":
            car.admin_note = note
        car.save(update_fields=["status", "admin_note", "updated_at", "published_at"])
